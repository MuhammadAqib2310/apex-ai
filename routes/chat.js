...
router.post('/chat', requireAuth, async (req, res) => {
  try {
    ...
    if (!GEMINI_API_KEY) {
      return res.status(500).json({ error: 'Server missing GEMINI_API_KEY. Add it to your Vercel environment variables.' });
    }
    ...
    // Call Gemini — non-streaming for Vercel compatibility
    ...
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${GEMINI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          system_instruction: { parts: [{ text: SYSTEM_PROMPT_NEW }] },
  ...
  try {
    ...
    const geminiRes = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${GEMINI_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          system_instruction: {
            parts: [{ text: `You are Apex AI — an expert trading analyst. 
